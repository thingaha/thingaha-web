import {
  ...
  export function* fetchAddressesSaga(action) {
    try {
      ...
      yield put({ type: GET_ALL_ADDRESSES_SUCCESS, addresses: json.data.addresses, count: json.data.count })
    } catch (error) {
  ...
  export function* searchAddressesSaga(action) {
    try {
      const json = yield fetchAddresses()
      yield put({ type: GET_SEARCH_ADDRESSES_SUCCESS, addresses: json.data.addresses })
    } catch (error) {
