...
export const fetchStudent = async (studentId) => {
  // const response = await axios.get(`${config.apiBaseUrl}/students/${studentId}`)
...
export const editStudent = (values) => {
  ...
  const deactivated_at = values.deactivated_at? values.deactivated_at : new Date()
  const editValues = { ...values, deactivated_at : values.isActivate? null : deactivated_at }

  return {
    data: [...studentsDb.filter((student) => student.id !== editValues.id), editValues],
  }
}
