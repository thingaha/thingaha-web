                ...
                <Grid item sm={3} xs={12} >
                    <Grid container spacing={1}>
                        <Grid item xs={12} style={{ paddingTop: '0.5rem' }}>
                            <NavLink to={link}>
                            <Typography variant="subtitle1" className="cardName">{address.addressable.name}</Typography>
                            </NavLink>
                        </Grid>
                        <Box display={{ xs: 'block', sm: 'none' }} style={{ marginLeft: '-2rem' }}>{icon}</Box>
                    </Grid>   
                </Grid>
                <Grid item sm={2} xs={6}>
                    <div xs={12} className="cardTitle">{labelDivision}</div>
                    <Typography variant="subtitle1" >{address.division}</Typography>
                </Grid>
                <Grid item sm={2} xs={6}>
                    <div xs={12} className="cardTitle">{labelDistrict}</div>
                    <Typography variant="subtitle1" >{address.district}</Typography>
                </Grid>
                <Grid item sm={2} xs={6}>
                    <div xs={12} className="cardTitle">{labelTownship}</div>
                    <Typography variant="subtitle1" >{address.township}</Typography>
                </Grid>
                <Grid item sm={3} xs={6}>
                    <div xs={12} className="cardTitle">{labelStreetAddress}</div>
                    <Typography variant="subtitle1" >{address.street_address}</Typography>
                    {/* <div xs={12}>{address.addressable.type}</div> */}
                </Grid>
                <Box display={{ xs: 'none', sm: 'block' }} style={{ marginRight: '-2rem' }}>{icon}</Box>
            </Grid>
